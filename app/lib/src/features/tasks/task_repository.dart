...
class TaskRepository {
  ...
  Future<Paginated<Task>> list({
    ...
    final json = await _api.get<Map<String, dynamic>>('/tasks', query: {
  ...
  Future<Task> update(int id, Map<String, dynamic> body) async {
    final json = await _api.patch<Map<String, dynamic>>('/tasks/$id', data: body);
    return Task.fromJson(json['data'] as Map<String, dynamic>);
  }
  ...
  Future<TaskComment> comment(int id, String comment) async {
    final json = await _api.post<Map<String, dynamic>>('/tasks/$id/comments', data: {
      'comment': comment,
    });
...
final taskRepositoryProvider =
    Provider((ref) => TaskRepository(ref.watch(apiClientProvider)));

/// Board filter state.
class TaskFilter {
  const TaskFilter({this.status, this.search = '', this.mineOnly = false, this.overdue = false});
